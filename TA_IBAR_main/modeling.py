...
def build_full_model(tokenizer, architecture: dict) -> Qwen3KimiAttnResForCausalLM:
    """Build TA-IBAR's own Full AttnRes pretraining model."""
    ...
    for key in ("num_hidden_layers", "hidden_size", "intermediate_size", "num_attention_heads", "num_key_value_heads", "max_position_embeddings"):
        if int(getattr(config, key)) != int(architecture[key]):
            raise RuntimeError(f"TA-IBAR architecture mismatch for {key}")
    native = Qwen3ForCausalLM(config)
    converted = Qwen3KimiAttnResForCausalLM(config_from_pretrained(config, mode="full"))
    ...
    if illegal or incompatible.unexpected_keys:
        raise RuntimeError(f"TA-IBAR Full conversion mismatch: missing={illegal}, unexpected={incompatible.unexpected_keys}")
...
def optimizer_factory(model: nn.Module, cfg: dict, stage: str) -> torch.optim.Optimizer:
    ...
    if not backbone or not query or not norm:
        raise RuntimeError("TA-IBAR optimizer groups are incomplete")
